...
-(BOOL)createDBWithName:(NSString *)name {
    ...
    if ([filemgr fileExistsAtPath: self.databasePath ] == NO) {
        if (self.logQuery) {[ChatManager logDebug:@"Database %@ not exists. Creating...", self.databasePath];}
        int result;
        ...
        if (self.logQuery) {[ChatManager logDebug:@"Database %@ already exists. Opening.", self.databasePath];}
...
// only for test
-(void)drop_database {
    if (self.logQuery) {[ChatManager logDebug:@"**** YOU DROPPED THE CHAT ARCHIVE: %@", self.databasePath];}
    NSFileManager *filemgr = [NSFileManager defaultManager];
    if ([filemgr fileExistsAtPath: self.databasePath ] == YES) {
        if (self.logQuery) {[ChatManager logDebug:@"**** DROPPED DATABASE %@", self.databasePath];}
        NSError *error;
        [filemgr removeItemAtPath:self.databasePath error:&error];
        if (error){
    ...
    dispatch_async(serialDatabaseQueue, ^{
        [self getConversationByIdSynchronized:conversation.conversationId completion:^(ChatConversation *conv_exists) {
            if (conv_exists) {
                ...
                BOOL ret = [self insertConversation:conversation];
...
-(BOOL)updateLastMessageInConversation:(NSString*)conversationId {
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        ...
        if (sqlite3_step(statement) == SQLITE_DONE) {
            ...
            [ChatManager logError:@"Error while updating conversation last message. Database returned error %d: %s", sqlite3_errcode(database), sqlite3_errmsg(database)];
        }
    }
...
-(BOOL)resetLastMessageInConversation:(NSString*)conversationId {
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        ...
        NSString *updateSQL = [NSString stringWithFormat:@"UPDATE conversations SET last_message_text = ' ' WHERE conversationId = ?"];
        if (self.logQuery) {[ChatManager logDebug:@"QUERY:%@", updateSQL];}
...
-(BOOL)removeMessage:(NSString*)messageId {
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK) {
        ...
        if (sqlite3_step(statement) == SQLITE_DONE) {
            ...
            [ChatManager logDebug:@"Error on removeConversation."];
            [ChatManager logDebug:@"Database returned error %d: %s", sqlite3_errcode(database), sqlite3_errmsg(database)];
        }
    }
